...
app.use((0, cors_1.default)({
    origin: [envSchema_1.validENV.FRONTEND_URL, "https://lost-and-found-portals.netlify.app"],
    credentials: true,
}));
...
app.use((err, req, res, next) => {
    if (err instanceof apiError_1.default) {
        return res
            ...
            .json(new apiResponse_1.default(false, err.statusCode, err.message, null, err.errors));
    }
    console.error("Unhandled error:", err);
    return res
        .status(500)
        .json(new apiResponse_1.default(false, 500, "Internal server error", null, null));
});
// 404 handler
app.use((req, res) => {
    res
