...
const googleAuthCallback = (req, res) => {
    try {
        ...
        if (!req.user) {
            return res.redirect(`${envSchema_1.validENV.FRONTEND_URL}/auth/error?message=Authentication failed`);
        }
        const user = req.user;
        // Validate user data
        if (!user.id || !user.email) {
            return res.redirect(`${envSchema_1.validENV.FRONTEND_URL}/auth/error?message=Invalid user data`);
        }
        ...
        res.redirect(`${envSchema_1.validENV.FRONTEND_URL}/auth/error?message=Authentication failed`);
    }
};
